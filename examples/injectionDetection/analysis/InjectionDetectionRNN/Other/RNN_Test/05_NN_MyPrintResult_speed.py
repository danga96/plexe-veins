...
class DataAnalysis:
    ...
    def get_stats_attack(self, attack):
        def _remove_negative(ds):
            return ds[ds > 0]
        DF_attack = pd.read_csv(self.test_path+attack)
        #print("--------------------------------------------",attack,"-------------------------------------")
        ...
        print("------------- ",attack," (TOT_SIM: ",_simulations,") -------------",
                "\n  attack_detect: ",attack_detect, " ({:.2f}%)".format(100 * attack_detect/_simulations), 
...
if __name__ == "__main__":
    ...
    #print(AllAttacks)
    scores = pool.map(analyzer.get_stats_attack, params)
