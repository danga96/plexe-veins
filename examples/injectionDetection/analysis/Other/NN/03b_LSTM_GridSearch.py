...
def create_model(dense_layer=0, dropout_rate=0.0, activation = 'relu', optimizer='Adam', learn_rate=0.001, init_mode='uniform'):
    # create model
    model = Sequential()
    """
    if dense_layer==0:
    ...
    """
    first_layer_size_LSTM= 16 if dense_layer==0 else dense_layer[0]
    last_layer_size_LSTM = 16 if dense_layer==0 else dense_layer[-1]
    ...
    for layer_size in dense_layer[1:-1]:
    ...
    optimizer = _get_optimizer(optimizer, learn_rate)
    # Compile model
...
dense_size_candidates = [(256, 128,), (64,64,)]
########################################################
