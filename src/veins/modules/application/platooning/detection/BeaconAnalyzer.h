...
public:
    BeaconAnalyzer(const PlatooningParameters& platooningParameters, const DetectionParameters& detectionParameters,
    ...
    void update(const std::shared_ptr<Plexe::VEHICLE_DATA>& predData, const std::shared_ptr<Plexe::VEHICLE_DATA>& follData,
                const std::shared_ptr<Plexe::RADAR_READING>& radarReading = nullptr);
...
private:
    ...
    std::pair<Eigen::VectorXd, Eigen::MatrixXd> updateKalmanFilter(
